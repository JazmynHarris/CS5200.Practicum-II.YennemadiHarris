...
DB_CERT <- < CERTIFICATE >
  ...
  dbExecute(mart,
            "CREATE INDEX idx_fact_client_project ON ExpenseFacts(ClientProjectDimID)")
  dbExecute(mart,
            "CREATE INDEX idx_fact_category ON ExpenseFacts(CategoryDimID)")
  dbExecute(mart,
            "CREATE INDEX idx_fact_currency ON ExpenseFacts(CurrencyDimID)")
  ...
  merge_from_sqlite <- function(sqliteDb, martDb) {
    ...
    fact_data <- merge(fact_data, clientProjectDim, by = c("ClientID", "ProjectID"))
    fact_data <- merge(fact_data,
                       categoryDim,
                       by = c("CategoryName", "SubCategoryName"))
    fact_data <- merge(fact_data, currencyDim, by = "CurrencyName")
